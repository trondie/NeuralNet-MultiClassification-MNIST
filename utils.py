...
def appendBiasColumnVector(mat: np.ndarray):
    height = mat.shape[0]
    column = np.ones((height, 1))
    return np.concatenate((mat, column), axis=1)
...
def improvedSigmoid(x):
    a = 0.002
    return 1.7159 * np.tanh((2/3) * x) + a * x

def improvedSigmoidDerivative(x): 
    a = 0.002
    return 1.7159 * (2/3) * (1 / np.cosh((2/3) * x) ** 2 ) + a 